...
- **Stateful stream processing:** Use Kafka Streams with RocksDB state store for consumer state management (deduplication, windowing, aggregation, joins). Do not use external datastores (Caffeine, Redis, MongoDB, PostgreSQL) for Kafka consumer state. State should be co-located with partition assignment and fault-tolerant via changelog topics.
