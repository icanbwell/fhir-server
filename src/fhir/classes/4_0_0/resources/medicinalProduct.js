...
class MedicinalProduct extends Resource {
    ...
    */
    ...
    async updateReferencesAsync({fnUpdateReferenceAsync}) {
            ...
            if (this.productClassification) { async.each(this.productClassification, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.marketingStatus) { async.each(this.marketingStatus, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.pharmaceuticalProduct) { async.each(this.pharmaceuticalProduct, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.packagedMedicinalProduct) { async.each(this.packagedMedicinalProduct, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.attachedDocument) { async.each(this.attachedDocument, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.masterFile) { async.each(this.masterFile, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.contact) { async.each(this.contact, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.clinicalTrial) { async.each(this.clinicalTrial, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.name) { async.each(this.name, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.crossReference) { async.each(this.crossReference, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.manufacturingBusinessOperation) { async.each(this.manufacturingBusinessOperation, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.specialDesignation) { async.each(this.specialDesignation, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
    }
