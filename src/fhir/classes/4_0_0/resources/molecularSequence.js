...
class MolecularSequence extends Resource {
    ...
    */
    ...
    async updateReferencesAsync({fnUpdateReferenceAsync}) {
            ...
            if (this.variant) { async.each(this.variant, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.quality) { async.each(this.quality, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.repository) { async.each(this.repository, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.pointer) { async.each(this.pointer, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.structureVariant) { async.each(this.structureVariant, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
    }
