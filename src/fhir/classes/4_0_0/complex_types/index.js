// This file is auto-generated by generate_classes so do not edit manually

const ageunits = require('./ageUnits');
const extension = require('./extension');
const backboneelement = require('./backboneElement');
const narrative = require('./narrative');
const element = require('./element');
const meta = require('./meta');
const address = require('./address');
const contributor = require('./contributor');
const attachment = require('./attachment');
const datarequirement = require('./dataRequirement');
const datarequirementcodefilter = require('./dataRequirementCodeFilter');
const datarequirementdatefilter = require('./dataRequirementDateFilter');
const datarequirementsort = require('./dataRequirementSort');
const money = require('./money');
const humanname = require('./humanName');
const contactpoint = require('./contactPoint');
const identifier = require('./identifier');
const ratiorange = require('./ratioRange');
const coding = require('./coding');
const sampleddata = require('./sampledData');
const ratio = require('./ratio');
const reference = require('./reference');
const triggerdefinition = require('./triggerDefinition');
const quantity = require('./quantity');
const period = require('./period');
const range = require('./range');
const relatedartifact = require('./relatedArtifact');
const annotation = require('./annotation');
const contactdetail = require('./contactDetail');
const usagecontext = require('./usageContext');
const expression = require('./expression');
const codeablereference = require('./codeableReference');
const signature = require('./signature');
const codeableconcept = require('./codeableConcept');
const parameterdefinition = require('./parameterDefinition');
const medicationstatuscodes = require('./medicationStatusCodes');
const medicationrequeststatus = require('./medicationrequestStatus');
const medicationrequestintent = require('./medicationRequestIntent');
const medicationstatementstatuscodes = require('./medicationStatementStatusCodes');
const messageheaderresponserequest = require('./messageheaderResponseRequest');

module.exports = {
    ageunits,
    extension,
    backboneelement,
    narrative,
    element,
    meta,
    address,
    contributor,
    attachment,
    datarequirement,
    datarequirementcodefilter,
    datarequirementdatefilter,
    datarequirementsort,
    money,
    humanname,
    contactpoint,
    identifier,
    ratiorange,
    coding,
    sampleddata,
    ratio,
    reference,
    triggerdefinition,
    quantity,
    period,
    range,
    relatedartifact,
    annotation,
    contactdetail,
    usagecontext,
    expression,
    codeablereference,
    signature,
    codeableconcept,
    parameterdefinition,
    medicationstatuscodes,
    medicationrequeststatus,
    medicationrequestintent,
    medicationstatementstatuscodes,
    messageheaderresponserequest,
};

