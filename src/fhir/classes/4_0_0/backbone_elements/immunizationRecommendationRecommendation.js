...
class ImmunizationRecommendationRecommendation extends Element {
    ...
    */
    ...
    async updateReferencesAsync({fnUpdateReferenceAsync}) {
            ...
            if (this.vaccineCode) { async.each(this.vaccineCode, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.targetDisease) {await this.targetDisease.updateReferencesAsync({fnUpdateReferenceAsync});}
            if (this.contraindicatedVaccineCode) { async.each(this.contraindicatedVaccineCode, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.forecastStatus) {await this.forecastStatus.updateReferencesAsync({fnUpdateReferenceAsync});}
            if (this.forecastReason) { async.each(this.forecastReason, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.dateCriterion) { async.each(this.dateCriterion, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.supportingImmunization) { async.each(this.supportingImmunization, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
            if (this.supportingPatientInformation) { async.each(this.supportingPatientInformation, async v => await v.updateReferences({fnUpdateReferenceAsync}));}
    }
