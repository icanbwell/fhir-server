...
def main():
    ...
    non_clinical_resource_fields_list_uuid = get_non_clinical_resources_fields(
        clinical_resources, use_uuid=True
    )

    # add custom field for accessing Binary resource from DocumentReference
    non_clinical_resource_fields_list_uuid["DocumentReference"].append(
        "content.attachment.url"
    )

    json_file_path = everything_operation.joinpath("generated.non_clinical_resources_fields.json")
    with open(json_file_path, "w") as json_file:
        json.dump(non_clinical_resource_fields_list_uuid, json_file, indent=2)
        json_file.write("\n")
