...
const SCRIPT_PATH = '../../../../admin/scripts/getMultipleOwnerDataCsv';
const RUNNER_PATH = '../../../../admin/runners/getMultipleOwnerDataCsvRunner';
...
async function runScript (parameters) {
    ...
    jest.doMock(RUNNER_PATH, () => ({ GetMultipleOwnerDataCsvRunner: MockRunner }));
...
describe('admin/scripts/getMultipleOwnerDataCsv.js', () => {
    afterEach(() => {
    ...
    test('batchSize uses the CLI parameter when supplied', async () => {
        const { MockRunner } = await runScript({ batchSize: 111 });
        expect(MockRunner.mock.calls[0][0].batchSize).toBe(111);
    });

    test('batchSize falls back to BULK_BUFFER_SIZE env var when parameter absent', async () => {
        process.env.BULK_BUFFER_SIZE = '4321';
        const { MockRunner } = await runScript({});
        expect(MockRunner.mock.calls[0][0].batchSize).toBe('4321');
    });
