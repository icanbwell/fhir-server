...
/**
 * getIncompatibleResources.js: thin CLI wrapper around GetIncompatibleResourcesRunner.
 */

const SCRIPT_PATH = '../../../../admin/scripts/getIncompatibleResources';
const RUNNER_PATH = '../../../../admin/runners/getIncompatibleResourcesRunner';
...
jestGlobal.mock(RUNNER_PATH, () => ({
    GetIncompatibleResourcesRunner: mockRunnerCtor
}));
...
describe('getIncompatibleResources.js (admin script)', () => {
    let exitSpy;
    let consoleLogSpy;
    let consoleErrorSpy;
    ...
    test('passes startFromCollection, limit, skip and startFromId through untouched', async () => {
        await runScript({ startFromCollection: 'Patient_4_0_0', limit: 1, skip: 2, startFromId: 'abc' });
        const options = mockRunnerCtor.mock.calls[0][0];
        expect(options.startFromCollection).toBe('Patient_4_0_0');
        expect(options.limit).toBe(1);
        expect(options.skip).toBe(2);
        expect(options.startFromId).toBe('abc');
    });

    test('wires databaseQueryFactory from the IoC container', async () => {
        await runScript({});
        expect(mockRunnerCtor.mock.calls[0][0].databaseQueryFactory).toEqual({ tag: 'databaseQueryFactory' });
    });
