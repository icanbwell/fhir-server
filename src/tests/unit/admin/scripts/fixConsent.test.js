...
/**
 * fixConsent.js: thin CLI wrapper around FixConsentRunner. Collections are hard-coded to
 * ['Consent_4_0_0'] -- there is no --collections CLI override at all for this script.
 */

const SCRIPT_PATH = '../../../../admin/scripts/fixConsent';
const RUNNER_PATH = '../../../../admin/runners/fixConsentRunner';
...
jestGlobal.mock(RUNNER_PATH, () => ({
    FixConsentRunner: mockRunnerCtor
}));
...
describe('fixConsent.js (admin script)', () => {
    let exitSpy;
    let consoleLogSpy;
    let consoleErrorSpy;
    ...
    test('always targets Consent_4_0_0, regardless of any --collections-like argv', async () => {
        await runScript({ collections: 'Patient_4_0_0' });
        expect(mockRunnerCtor.mock.calls[0][0].collections).toEqual(['Consent_4_0_0']);
    });
    ...
    test('wires bwellPersonFinder and preSaveManager from the IoC container', async () => {
        await runScript({});
        const options = mockRunnerCtor.mock.calls[0][0];
        expect(options.bwellPersonFinder).toEqual({ tag: 'bwellPersonFinder' });
        expect(options.preSaveManager).toEqual({ tag: 'preSaveManager' });
    });

    test('awaits runner.processAsync() then exits 0 on success, logging "Exiting process fixConsent"', async () => {
        await runScript({});
        expect(mockProcessAsync).toHaveBeenCalledTimes(1);
        expect(exitSpy).toHaveBeenCalledWith(0);
        expect(mockAdminLoggerCtor.mock.results[0].value.logInfo).toHaveBeenCalledWith('Exiting process fixConsent');
    });
});
