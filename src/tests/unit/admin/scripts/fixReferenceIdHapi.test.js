...
/**
 * fixReferenceIdHapi.js: CLI wrapper around FixReferenceIdHapiRunner. Mirrors fixReferenceId.js's
 * shape but with `hapiCollections` (built from the hapiResources list, base + _History variants)
 * instead of `proaCollections`.
 */

const SCRIPT_PATH = '../../../../admin/scripts/fixReferenceIdHapi';
const RUNNER_PATH = '../../../../admin/runners/fixReferenceIdHapiRunner';
...
jestGlobal.mock(RUNNER_PATH, () => ({
    FixReferenceIdHapiRunner: mockRunnerCtor
}));
...
describe('fixReferenceIdHapi.js (admin script)', () => {
    let exitSpy;
    let consoleLogSpy;
    let consoleErrorSpy;
    ...
    test('defaults hapiCollections to the built-in list expanded with both base and _History variants', async () => {
        await runScript({});
        ...
        expect(options.proaCollections).toContain('ServiceRequest_4_0_0');
        expect(options.proaCollections).toContain('ServiceRequest_4_0_0_History');
    });

    test('an explicit --hapiCollections replaces the default expansion entirely', async () => {
        await runScript({ hapiCollections: 'Patient_4_0_0' });
