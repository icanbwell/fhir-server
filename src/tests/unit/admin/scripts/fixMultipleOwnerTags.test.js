...
/**
 * fixMultipleOwnerTags.js: same thin-wrapper shape as fixDuplicateOwnerTags.js / fixInstantDataType.js
 * (parse argv -> defaulted options -> construct FixMultipleOwnerTagsRunner via the IoC container ->
 * await processAsync() -> exit(0)). No module.exports; self-executes on require.
 */

const SCRIPT_PATH = '../../../../admin/scripts/fixMultipleOwnerTags';
const RUNNER_PATH = '../../../../admin/runners/fixMultipleOwnerTagsRunner';
...
jestGlobal.mock(RUNNER_PATH, () => ({
    FixMultipleOwnerTagsRunner: mockRunnerCtor
}));
...
describe('fixMultipleOwnerTags.js (admin script)', () => {
    let exitSpy;
    let consoleLogSpy;
    let consoleErrorSpy;
    ...
    test('splits and trims a comma-separated --collections', async () => {
        await runScript({ collections: 'Person_4_0_0 , Patient_4_0_0' });
        expect(mockRunnerCtor.mock.calls[0][0].collections).toEqual(['Person_4_0_0', 'Patient_4_0_0']);
    });
    ...
    test('an explicit --batchSize overrides the default', async () => {
        await runScript({ batchSize: 42 });
        expect(mockRunnerCtor.mock.calls[0][0].batchSize).toBe(42);
    });
    ...
    test('passes startFromCollection, limit, skip and startFromId through untouched', async () => {
        await runScript({ startFromCollection: 'Person_4_0_0', limit: 1, skip: 2, startFromId: 'id-1' });
        ...
        expect(options.startFromId).toBe('id-1');
    });
