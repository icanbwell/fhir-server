...
const { makeStatement, securityStatement } = require('../../../../../middleware/fhir/metadata/capability.1_0_2');
const { resolveSchema } = require('../../../../../middleware/fhir/utils/schema.utils');

describe('capability.1_0_2', () => {
    beforeEach(() => {
    ...
    describe('makeStatement', () => {
        test('creates a Conformance with correct status', () => {
        ...
        test('creates a Conformance with formatted date', () => {
            const resources = { mode: 'server', resource: [] };
            const result = makeStatement(resources);
        ...
        test('sets implementation description for DSTU2', () => {
            const resources = { mode: 'server', resource: [] };
            const result = makeStatement(resources);
        ...
        test('sets fhirVersion to 1.0.2', () => {
            const resources = { mode: 'server', resource: [] };
            const result = makeStatement(resources);

            expect(result.fhirVersion).toBe('1.0.2');
        });
        ...
        test('sets format to application/json+fhir', () => {
            const resources = { mode: 'server', resource: [] };
            const result = makeStatement(resources);

            expect(result.format).toEqual(['application/json+fhir']);
        });
        ...
        test('resolves schema with version 1_0_2 and conformance', () => {
            const resources = { mode: 'server', resource: [] };
            makeStatement(resources);

            expect(resolveSchema).toHaveBeenCalledWith('1_0_2', 'conformance');
        });
    });
