...
const expectedPatientClinicalSummary = require('./fixtures/expected/expected_Patient_clinical_summary.json');
const expectedPatientInsurance = require('./fixtures/expected/expected_Patient_clinical_insurance.json');
...
describe('Person and Patient real chatgpt Tests', () => {
    ...
    describe('Person and Patient chatgpt Tests', () => {
        ...
        test('Patient with clinical summary', async () => {
            console.log(`OPENAI_API_KEY=${process.env.OPENAI_API_KEY}`);
            if (!process.env.OPENAI_API_KEY) {
                return;
            }
            ...
            const urlEncodedQuestion = encodeURIComponent(
                'Write a clinical summary in prose for this patient to show to a doctor for a second opinion.'
            );
        ...
        test('Patient with insurance plan', async () => {
            ...
            resp = await request
                .post('/4_0_0/DocumentReference/123456/$merge?validate=true')
            ...
            resp = await request
                ...
                .set(getHeaders());
            // noinspection JSUnresolvedFunction
            expect(resp).toHaveResponse(expectedPatientInsurance, (resource) => {
                const currentDate = new Date().toISOString().split('T')[0];
