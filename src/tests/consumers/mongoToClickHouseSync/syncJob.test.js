...
describe('SyncJob', () => {
    ...
    beforeEach(async () => {
        ...
        mockCheckpointManager = {
            ...
            updateCheckpointAsync: jest.fn(async (checkpointKey, lastMongoId, lastUpdated) => {
                checkpoints.push({ checkpointKey, lastMongoId, lastUpdated });
            }),
        ...
        profile = createResourceHistorySyncProfile({
    ...
    test('should process a single batch of documents', async () => {
        ...
        await job.executeAsync({ jobId: 'test-job', resourceType: 'Patient' }, profile);
    ...
    test('should throw when ClickHouse insert fails after retries', async () => {
        ...
        await expect(
            job.executeAsync({ jobId: 'test-job', resourceType: 'Patient' }, profile)
    ...
    test('should not delete from MongoDB when deleteFromMongo is false', async () => {
        const docs = [createHistoryDoc(0)];
        mockCollection.find = jest.fn(() => createMockCursor(docs));
    ...
    test('should delete from MongoDB when deleteFromMongo is true', async () => {
    ...
    test('should use checkpoint when resuming', async () => {
        ...
        mockCheckpointManager.getCheckpointAsync = jest.fn(async (checkpointKey) => {
            expect(checkpointKey).toBe('Patient');
            return {
