...
class MockConfigManagerRawGraphWithSerializer extends ConfigManager {
    get enableFastSerializerInGraphOp() {
        return true;
    }
}
describe('Person graph_person_to_patient_to_task Tests with fast serializer', () => {
    beforeEach(async () => {
    ...
    test('graph_person_to_patient_to_task works (with reference id and sourceAssigningAuthority)', async () => {
        ...
        const request = await createTestRequest((container) => {
            container.register('configManager', () => new MockConfigManagerRawGraphWithSerializer());
            return container;
        });
        ...
        // ACT & ASSERT
        resp = await request
            .post('/4_0_0/Person/$graph?id=002126287fbd412d8b52115e48edbd4c&contained=true')
            .set(getHeaders())
    ...
    test('graph_person_to_patient_to_task works (with proxy patient)', async () => {
        ...
        // ACT & ASSERT
        resp = await request
            .post('/4_0_0/Patient/$graph?id=person.002126287fbd412d8b52115e48edbd4c&contained=true&_debug=1')
            .set(getHeaders())
