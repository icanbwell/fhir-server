...
class ResourceHistoryTransformer {
    /**
    ...
    transform(doc) {
        if (!doc || !doc.resource || !doc.resource.resourceType) {
            logError('ResourceHistoryTransformer: skipping malformed document', {
        ...
        if (lastUpdated instanceof Date) {
            ...
            logError('ResourceHistoryTransformer: missing lastUpdated', {
                args: { mongoId: doc._id?.toString() }
            });
            return null;
        }
...
module.exports = { ResourceHistoryTransformer };
