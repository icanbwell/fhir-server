...
if [ "$1" = "otel" ]; then
    OTEL_NODE_ENABLED_INSTRUMENTATIONS=dataloader,express,graphql,lru-memoizer,router,winston,http,mongodb,redis exec node --max-old-space-size=$MAX_OLD_SPACE_SIZE_MB --require=./.pnp.cjs --require=./src/otel_instrumentation.js src/index.js
else
    exec node --max-old-space-size=$MAX_OLD_SPACE_SIZE_MB --require=./.pnp.cjs src/index.js
fi
