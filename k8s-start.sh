...
if [ "$1" = "otel" ]; then
    OTEL_NODE_ENABLED_INSTRUMENTATIONS=dataloader,express,graphql,lru-memoizer,router,winston,http,mongodb,redis,runtime-node,aws-sdk,kafkajs, exec node --max-old-space-size=$PER_WORKER_MAX_OLD_SPACE_SIZE_MB --require=./src/otel_instrumentation.js src/index.js
else
