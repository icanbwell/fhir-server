...
Add v2 CRUDS granularity to `patient/`, `user/`, and `access/` scope prefixes, all while continuing
to fully support v1 `read`/`write`/`*` grammar (mixed v1/v2 scopes in the same token/request must
both work, since SMART permits a client to hold either style and this server has live v1-only
clients today).
...
- `system/` scopes (backend-services, no `access/`-tag interaction) — separate future effort.
...
`access/` scope string into a structured shape:

```
{ prefix: 'patient'|'user'|'access', resourceType: string, cruds: Set<'c'|'r'|'u'|'d'|'s'> }
```
...
- **`scopesManager.js`** — `getAccessCodesFromScopes`, `getPatientScopes`, `getUserScopes` call
  into `smartScopeParser` instead of doing raw `.`-split / prefix-filtering inline. Public function
...
1. New isolated unit tests for `smartScopeParser.js`: v1→CRUDS normalization table, v2 grammar
   detection (valid combinations, duplicates, unknown letters, empty suffix), all three prefixes.
