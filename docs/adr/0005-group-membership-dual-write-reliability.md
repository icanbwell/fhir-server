...
- **How members are retrieved (two distinct operations).** (1) *Membership testing* ("is patient X in a group / which groups contain X") uses the standard `member` search parameter (`GET /Group?member=X`) — a reverse lookup returning a cursor-paged searchset Bundle; already implemented and tested. It does not enumerate a given group's roster. (2) *Cohort enumeration* ("list all members of Group G") is the Bulk Data `Group/[id]/$export` operation (async `202` + `Content-Location` → poll → NDJSON manifest; server-side expansion backed by the ClickHouse roster) — the standard, interoperable mechanism, and the one that scales to the ~10M-member cohorts the DQM pipeline targets. Not yet wired here (tracked as EA-2331). The member count is read from `quantity`. A custom `$members` operation is deliberately not introduced: it is non-standard, and the external DQM contract (Confluence "DQM External API" TDD) already commits to the standard DEQM `$evaluate` with a `subject`/`subjectGroup` reference and "no custom REST alternative — we follow the standard." `Group.member[]` is never returned inline on a synchronous read (see ADR 0004), consistent with the Bulk Data server-side-expansion model. Surfacing these in the generated CapabilityStatement is tracked separately (EA-2332; the current generator emits no per-resource/search documentation).
...
- **Downstream measure evaluation must not read `Group.member[]` inline.** b.well resolves a cohort to patients upstream — the `clinical-reasoning-orchestrator` fans out per-person work units and the CQL engine (`bsights-engine-cql`) evaluates one Person at a time via cqframework `LibraryEngine` — so stripped `member[]` does not affect current evaluation. If a stock cqframework/HAPI `MeasureProcessor` `$evaluate-measure(subject=Group)` is ever introduced, it would call `repository.read(Group).member` and receive an empty array; that path must resolve the Group via `Group/$export` (EA-2331) or a pre-resolved patient list, never a synchronous inline read.
...
- [ADR 0004](0004-clickhouse-group-causal-ordering.md) (strip-on-read + causal ordering + version bump)
- FHIR R4 `http.html` (update/read semantics, versioning), `search.html` (search currency)
- Bulk Data Access IG `Group/[id]/$export` (server-side cohort expansion): https://build.fhir.org/ig/HL7/bulk-data/en/export.html
- DQM Platform TDD (Confluence 5678235768) and DQM External API TDD (Confluence 6181158948) — the measure-evaluation consumers; cohort enumeration should use `$export`, not a custom `$members`
- EA-2331 (async `Group/[id]/$export` backed by the ClickHouse roster)
