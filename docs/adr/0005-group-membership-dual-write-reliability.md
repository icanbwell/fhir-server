...
- **How members are retrieved.** Membership is queried via the standard `member` search parameter (`GET /Group?member=X`), which is cursor-paged (a searchset Bundle with `next` links) and already implemented and tested; the member count is read from `quantity`. Because `member` is not returned inline on read, there is no standard `$members` operation; bulk retrieval of a Group's member resources is the Bulk Data `Group/[id]/$export` operation, not yet wired here (tracked as EA-2331). Surfacing this behavior in the generated CapabilityStatement is tracked separately (the current generator emits no per-resource/search documentation). This ADR does not depend on that work; it is noted so the read model is complete.
