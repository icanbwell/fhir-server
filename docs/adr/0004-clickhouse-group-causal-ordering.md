...
1. **Advance the version on a membership change (Option B).** In the Group update path (`src/operations/update/update.js`), before the merge, hydrate `currentResource.member` from ClickHouse (`GroupMemberRepository.getActiveMembers`, reconstructing minimal `{ entity: { reference } }` entries) when — and only when — **the incoming write carries a `member` field** (present, even `[]`) **and** the resource is a hybrid Group: `configManager.enableClickHouse` **and** `configManager.mongoWithClickHouseResources.includes('Group')` **and** the stored document carries the external-storage member tag (`system: https://www.icanbwell.com/externalStorageFields`, `code: member`) **and** the repository is wired. Everything about the hook no-ops when ClickHouse is off or the resource is not a hybrid Group. The repository is injected through the DI container (`src/createContainer.js`) and returns `null` when ClickHouse is disabled.

   The **incoming-`member` gate is load-bearing**: a `PUT` is a full replace, so an absent `member` field means "no roster asserted", not "empty roster". Hydrating there would make the diff read as *remove all* and wipe a populated roster, and it would also spuriously bump the version on a metadata-only `PUT`. Two things enforce "member not asserted ⇒ roster untouched": the update path does not hydrate (so the content diff and version are unaffected by membership), and it threads `memberFieldPresent: false` into `contextData` so the post-save ClickHouse handler skips membership processing (rather than diffing the current roster against an empty `doc.member` and emitting removals). So: a `PUT` that **omits** `member` leaves the roster and its events untouched and bumps the version only if other content changed (its pre-existing behavior); `member: []` hydrates and clears the roster (bump); a member-bearing `PUT` hydrates and diffs against the current roster (bump iff it differs). The read path (`GroupMemberEnrichmentProvider`) returns `quantity` and strips `member`, so a faithful GET→PUT round-trip carries no `member` and correctly does not hydrate, bump, or emit events.
...
- **One extra ClickHouse read on a member-bearing hybrid-Group `PUT`.** The hook issues a single `getActiveMembers` roster read before the merge, only when the write carries `member`. It is skipped for a `PUT` that omits `member`, for metadata-only pure-Mongo Groups, and for all non-Group resources. Deduplicating this against the post-save handler's own `getActiveMembers` read is a possible future optimization, not done here.
- **Residual over-bump — explicit unchanged-roster resend.** If a client sends an explicit `member` array equal to the current roster, the version bumps only if the arrays differ under a positional JSON diff. `getActiveMembers` uses `ORDER BY entity_reference` so hydration is deterministic, and the reconstructed shape (`{ entity: { reference } }`) matches what a client can round-trip (the read path returns only `quantity`, not `member`). A resend in a *different order* than stored may still diff and bump the version; there is no data corruption (the post-save membership diff is Set-based, so only the version churns). Normalizing arbitrary client member order before diffing is out of scope.
...
- Code: `src/operations/update/update.js` (hydrate-before-merge version bump, gated on the incoming member field), `src/utils/contextDataBuilder.js` + `src/dataLayer/postSaveHandlers/clickHouseGroupHandler.js` (`memberFieldPresent` skip), `src/dataLayer/repositories/groupMemberRepository.js` (`getActiveMembers`, ordered), `src/utils/clickHouseGroupPreSave.js` (external-storage tag + member strip), `src/dataLayer/builders/groupMemberEventBuilder.js`, `src/utils/clickHouse/queryFragments.js`
